...
internal class CrateMachineDriver : MonoBehaviour
{
    ...
    public void Initialize(ACrateMachine crateMachine)
    {
...
public class CrateMachine<TParent> : ACrateMachine
{
    ...
    public CrateMachine<TParent> AddCrate<TCrate>(TCrate crate, bool initial = false) where TCrate : Crate<TParent>
    {
        crates[typeof(TCrate)] = crate;
        crate.SetParent(parent);
        if (initial) SetInitialCrate<TCrate>();
        return this;
    }
    ...
    public CrateMachine<TParent> Build()
    {
        ...
        if (crates.Count == 0)
            throw new InvalidOperationException($"{machineType} for {parentName} has no crates");

        if (initialCrateType == null)
            throw new InvalidOperationException($"{machineType} for {parentName} has no initial crate set");

        if (driver != null)
            throw new InvalidOperationException($"{machineType} is already built for {parentName}");

        currentCrate = crates[initialCrateType];
        driver = new GameObject($"{parentName} driver (for {machineType})");
        driver.AddComponent<CrateMachineDriver>().Initialize(this);
        return this;
    }
