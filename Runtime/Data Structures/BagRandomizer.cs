...
namespace crass
{
    ...
    public class BagRandomizer<T>
    {
        ...
        [Tooltip(
            "If this is true and every value in Items is unique, GetNext will never return the same value twice."
        )]
        public bool AvoidRepeats;

        [Tooltip(
            "Sets which PRNG to use. Global uses the global UnityEngine.Random, while the Local values instantiate one PRNG per BagRandomizer. If set to LocalSeeded, the LocalSeed value is used; if set to Local, an arbitrary seed is chosen."
        )]
        ...
        System.Random localRandom
        {
            get
            {
                if (_localRandom == null)
                    _localRandom = new System.Random(
                        Type == PRNGType.LocalSeeded ? LocalSeed : Environment.TickCount
                    );
