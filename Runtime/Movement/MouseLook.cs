...
namespace crass
{
    public class MouseLook : MonoBehaviour
    {
        ...
        [Tooltip(
            "Higher values mean less jittery input when moving the mouse, but adds lag and makes input less precise. Set to (1, 1) to remove all smoothing."
        )]
        public Vector2 Smoothing = new Vector2(1, 1);

        [Tooltip(
            "Assign this if there's a parent object controlling motion, such as a Character Controller. Yaw rotation will affect this object instead of the camera if set."
        )]
        ...
        void Update()
        {
            ...
            transform.localRotation =
                Quaternion.AngleAxis(-mousePosition.y, targetOrientation * Vector3.right)
                * targetOrientation;
            ...
            {
                var yRotation = Quaternion.AngleAxis(
                    mousePosition.x,
